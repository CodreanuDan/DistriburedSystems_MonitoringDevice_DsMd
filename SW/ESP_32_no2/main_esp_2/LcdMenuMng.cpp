...
void LcdMng_Init()
{
    ...
    /* Cusotm chars */
    lcd.createChar(0, customCharsByte.motor);   // Motor
    lcd.createChar(1, customCharsByte.hum);     // Humidity
    lcd.createChar(2, customCharsByte.mqtt);    // Mqtt
    lcd.createChar(3, customCharsByte.temp);    // Temp
...
void LcdMng_BuildMenu()
{
    ...
    strncpy(n2.text, "Bme680 Screen", sizeof(n2.text) - 1);
...
void LcdMng_Set_SCREEN_STATUS()
{
    ...
    // Row 2: Light, MQTT status
    lcd.setCursor(0, 1);
    lcd.write(2);  // MQTT icon
    lcd.print(":");
    lcd.write(g_systemValues.wifi.mqtt_connected ? 6 : 7);  // Assuming MQTT follows WiFi
    
    lcd.print(" ");  
    lcd.write(5);  // Light bulb icon
    lcd.print(":");
    lcd.write(g_systemValues.led.toggled ? 6 : 7);  // ✗ or ✓
    lcd.print(":");
    lcd.print(g_systemValues.mqtt.overdrive ? "MAN" : "AUT");
    lcd.print(":");  
    lcd.print(g_systemValues.led.pwm);
    lcd.print("%");
}

void LcdMng_Set_SCREEN_BME()
{
    /*  Row 1: Temp and  Hum*/
    lcd.setCursor(0, 0);

    lcd.write(3);
    lcd.print(g_systemValues.bme.temp, 2);
    lcd.print("C ");

    lcd.write(1);
    lcd.print(g_systemValues.bme.hum);
    lcd.print("% ");
    ...
    lcd.print("IAQ:");
    lcd.print(uint8_t(g_systemValues.bme.iaq));
    lcd.print(" ");
    
    lcd.print("ITU:");
    lcd.print(g_systemValues.bme.itu, 1);
